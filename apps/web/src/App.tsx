...
import { Boxes, Cable, Check, DownloadCloud, Gauge, Home, Lightbulb, Link, Play, RotateCw, Save, Settings2, Square, Wifi } from "lucide-react";
...
type Tab = "overview" | "devices" | "connections" | "runtime";
...
export default function App() {
  ...
  const pair = async () => { setBusy(true); try { const next = await pairHomeAssistant(companionUrl, pairingCode); setSettings(next); setPairingCode(""); setNotice("Home Assistant and MQTT are configured"); } catch (e) { setNotice(String(e)); } finally { setBusy(false); } };
  ...
  return <div className="shell">
    ...
    <main>
      <header><div><p>DESKTOP CONTROL CENTER</p><h1>{tab === "overview" ? "Your lighting, in sync." : tab === "devices" ? "iCUE devices" : tab === "connections" ? "Connections" : "Runtime"}</h1></div><button className="primary" disabled={busy} onClick={persist}><Save size={17}/>{busy ? "Working…" : "Save changes"}</button></header>
      ...
      {tab === "devices" && <section className="panel wide"><div className="panel-title"><div className="icon cyan"><Boxes/></div><div><h2>{inventory.count} detected device{inventory.count === 1 ? "" : "s"}</h2><p>{inventory.connected ? `Grouped in Home Assistant under ${settings.suggestedArea}.` : "Start HomeCue to refresh the iCUE inventory."}</p></div></div><div className="device-list">{inventory.devices.length === 0 ? <p className="empty">No device inventory is available yet.</p> : inventory.devices.map(device => <article key={device.id}><div><strong>{device.name}</strong><small>{device.model} · {device.type}</small></div><div className="device-meta"><span>{device.ledCount} LEDs</span>{device.capabilities.map(capability => <span key={capability}>{capability}</span>)}</div></article>)}</div></section>}
      {tab === "connections" && <><section className="panel setup-panel"><div className="panel-title"><div className="icon cyan"><Link/></div><div><h2>Automatic setup</h2><p>Pair with the HomeCue Companion app in Home Assistant.</p></div></div><div className="pairing-form"><Field label="Home Assistant address"><input type="url" value={companionUrl} onChange={e => setCompanionUrl(e.target.value)}/></Field><Field label="One-time pairing code"><input className="pairing-code" autoComplete="one-time-code" placeholder="XXXX-XXXX-XXXX-XXXX" value={pairingCode} onChange={e => setPairingCode(e.target.value.toUpperCase())}/></Field><button className="primary" disabled={busy || pairingCode.replace(/[-\s]/g, "").length !== 16} onClick={pair}><Link size={16}/>Connect automatically</button></div><small className="setup-hint">Open the companion's Web UI in Home Assistant to get a ten-minute code. Requires Home Assistant OS or Supervised.</small></section><div className="section-label">Manual configuration</div><section className="panel-grid">
