...
import { Boxes, Check, DownloadCloud, ExternalLink, Gauge, Home, Lightbulb, Link, Play, RotateCw, Save, Settings2, Square, Wifi } from "lucide-react";
import { checkForUpdates, defaults, getIcueStatus, getServiceStatus, installUpdate, launchIcue, loadInventory, loadSettings, openIcueDownload, pairHomeAssistant, saveSettings, startService, stopService, type AppUpdate, type IcueStatus, type Inventory, type ServiceStatus, type Settings } from "./bridge";
...
export default function App() {
  ...
  const [icue, setIcue] = useState<IcueStatus>({ installed: false, running: false, active: false, message: "Checking for iCUE…" });
  ...
  const startIcue = async () => { setBusy(true); try { await launchIcue(); setNotice("iCUE is starting…"); window.setTimeout(() => void refresh(), 3500); } catch (e) { setNotice(String(e)); } finally { setBusy(false); } };
  ...
  const updateApp = async () => { setBusy(true); setNotice("Stopping HomeCue before updating…"); try { setStatus(await stopService()); setNotice("Downloading the update…"); await installUpdate(); } catch (e) { setNotice(`Update failed: ${String(e)}`); setBusy(false); } };
  ...
  return <div className="shell">
    ...
    <main>
      ...
      {tab === "devices" && <><section className={`panel wide icue-state ${icue.active ? "active" : ""}`}><div><span className={`service-dot ${icue.active ? "online" : ""}`}/><div><h2>{icue.message}</h2><p>{!icue.installed ? "Install Corsair iCUE to discover and control supported hardware." : !icue.running ? "HomeCue can launch the detected installation for you." : !icue.active ? "Enable SDK access in iCUE Settings, then scan again." : "The iCUE SDK connection is ready for device discovery."}</p></div></div>{!icue.installed ? <button className="primary" onClick={openIcueDownload}><ExternalLink size={16}/>Download iCUE</button> : !icue.running ? <button className="primary" disabled={busy} onClick={startIcue}><Play size={16}/>Launch iCUE</button> : <button className="secondary" disabled={busy} onClick={refreshDevices}><RotateCw size={16}/>Refresh status</button>}</section><section className="panel wide"><div className="panel-title device-title"><div className="icon cyan"><Boxes/></div><div><h2>{inventory.count} detected device{inventory.count === 1 ? "" : "s"}</h2><p>{inventory.connected ? `Grouped in Home Assistant under ${settings.suggestedArea}.` : "Start HomeCue to refresh the iCUE inventory."}</p></div><button className="secondary" disabled={busy} onClick={refreshDevices}><RotateCw size={16}/>{busy ? "Scanning…" : "Scan for devices"}</button></div><div className="device-list">{inventory.devices.length === 0 ? <p className="empty">{icue.active ? "iCUE is connected but reported no devices. Confirm the hardware appears in iCUE, then scan again." : "Device discovery will begin after iCUE is installed, running, and SDK access is enabled."}</p> : inventory.devices.map(device => <article key={device.id}><div><strong>{device.name}</strong><small>{device.model} · {device.type}</small></div><div className="device-meta"><span>{device.ledCount} LEDs</span>{device.capabilities.map(capability => <span key={capability}>{capability}</span>)}</div></article>)}</div></section></>}
